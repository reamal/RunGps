...
public class WatchDogService extends Service {
    ...
    int onStart(Intent intent, int flags, int startId) {
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.N) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) startService(new Intent(App.getInstance(), WatchDogNotificationService.class));
        }

        if (disposable != null && !disposable.isDisposed()) return START_STICKY;

        //定时检查 WorkService 是否在运行，如果不在运行就把它拉起来
        //Android 5.0+ 使用 JobScheduler，效果比 AlarmManager 好
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            JobInfo.Builder builder = new JobInfo.Builder(HASH_CODE, new ComponentName(App.getInstance(), JobSchedulerService.class));
            ...
            Intent i = new Intent(App.getInstance(), WorkService.class);
            sPendingIntent = PendingIntent.getService(App.getInstance(), HASH_CODE, i, PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        disposable = Observable.interval(INTERVAL_WAKE_UP, TimeUnit.MILLISECONDS)
                 .subscribe(aLong -> startService(new Intent(App.getInstance(), WorkService.class)), Throwable::printStackTrace);
    ...
    void onEnd(Intent rootIntent) {
        ...
        startService(new Intent(App.getInstance(), WatchDogService.class));
    }
    ...
    public static void cancelJobAlarmSub() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            JobScheduler scheduler = (JobScheduler) App.getInstance().getSystemService(JOB_SCHEDULER_SERVICE);
